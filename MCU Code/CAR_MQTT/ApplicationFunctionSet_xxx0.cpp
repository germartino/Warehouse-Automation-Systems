...
int global_path_finder;
boolean maybeReturn = false;
boolean almostReturn = false;
...
int ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void)
{
...
void ApplicationFunctionSet::car_control(String control, int car_speed)
{
  if (control == "Forward") {
...
int ApplicationFunctionSet::find_path(char control, int path_finder)
{
  global_path_finder = path_finder;
  float getAnaloguexxx_L = AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_L();
  float getAnaloguexxx_M = AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_M();
  float getAnaloguexxx_R = AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_R();
...
int ApplicationFunctionSet::moveIt(char control, int car_direction, int car_distance, int car_speed_hi)
{
  //After following the path and stopping for n seconds, go forward to take the RETURN PATH
  Application_FunctionSet.car_control("Forward", car_speed_hi);
  delay(car_distance*40);
  Application_FunctionSet.car_control("stop_it", 0);
  delay(2000);
  return 0;
}
